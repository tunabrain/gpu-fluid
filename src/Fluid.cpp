...
Fluid::Fluid(int width, int height) : _width(width), _height(height) {
    ...
    Texture **ts[] = {&_u, &_v, &_d, &_t, &_aDiag, &_aPlusX, &_aPlusY, &_p, &_r, &_z, &_s, &_tmp1, &_tmp2, &_uTmp, &_vTmp, &_tTmp, &_dTmp};
    for (int i = 0; i < 17; i++) {
...
void Fluid::testAddReduce() {
    ...
    addReduce(*_tmp1, *_dotPTransfer[0]);

    float res[4];
    _dotPTransfer[0]->bindAny();
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RED, GL_FLOAT, res);

    float total = res[0] + res[1] + res[2] + res[3];

    printf("Add reduce error: %f\n", fabs(total - dot));
    fflush(stdout);
}

void Fluid::testMaxReduce() {
    float *data = new float[_width*_height];
    ...
    float res = maxReduce(*_tmp1, *_dotPTransfer[0]);

    printf("Max reduce error: %f\n", fabs(res - m));
    fflush(stdout);
}

void Fluid::testMatVecP() {
    float *data1 = new float[_width*_height];
    float *data2 = new float[_width*_height];
    float *data3 = new float[_width*_height];
    float *data4 = new float[_width*_height];

    float *res = new float[_width*_height];

    for (int i = 0; i < _width*_height; i++) {
        data1[i] = frand()*2.0 - 1.0;
        data2[i] = frand()*2.0 - 1.0;
        data3[i] = frand()*2.0 - 1.0;
        data4[i] = frand()*2.0 - 1.0;
    }

    int w = _width, h = _height;
    for (int y = 0, idx = 0; y < _height; y++) {
        for (int x = 0; x < _width; x++, idx++) {
            res[idx] = data1[idx]*data4[idx];

            if (x > 0)     res[idx] += data2[idx - 1]*data4[idx - 1];
            if (y > 0)     res[idx] += data3[idx - w]*data4[idx - w];
            if (x < w - 1) res[idx] += data2[idx    ]*data4[idx + 1];
            if (y < h - 1) res[idx] += data3[idx    ]*data4[idx + w];
        }
    }
    ...
    //matVecProduct(*_aDiag, *_aPlusX, *_aPlusY, *_u, *_v);

    _v->bindAny();
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RED, GL_FLOAT, data1);

    double rmsq = 0.0;
    for (int i = 0; i < _width*_height; i++) {
        float r = res[i] - data1[i];

        rmsq += r*r;
    }

    printf("Mat vec product RSQ: %f\n", rmsq);
    fflush(stdout);
}

void Fluid::testAddSub() {
    float *data1 = new float[_width*_height];
    float *data2 = new float[_width*_height];
    float *data3 = new float[_width*_height];
    float *data4 = new float[_width*_height];
    ...
    for (int y = 0, idx = 0; y < _height; y++) {
        for (int x = 0; x < _width; x++, idx++) {
            data1[idx] = (x < _width - 1 && y < _height - 1 ? frand()*2.0 - 1.0 : 0.0);
            data2[idx] = (x < _width - 1 && y < _height - 1 ? frand()*2.0 - 1.0 : 0.0);
            data3[idx] = (x < _width - 1 && y < _height - 1 ? frand()*2.0 - 1.0 : 0.0);
            data4[idx] = (x < _width - 1 && y < _height - 1 ? frand()*2.0 - 1.0 : 0.0);
        }
    }

    float alphas[] = {frand()*2.0 - 1.0, 0.0, 0.0, 0.0, frand()*2.0 - 1.0, 0.0, 0.0, 0.0};

    for (int i = 0; i < 2; i++)
        _dotPTransfer[i]->copy(&alphas[i*4]);
    ...
    for (int i = 0; i < _width*_height; i++) {
        data1[i] += alphas[0]/alphas[4]*data3[i];
        data2[i] -= alphas[0]/alphas[4]*data4[i];
    }

    addSub(*_v, *_t, *_u, *_d, *_tmp1, *_tmp2, *_dotPTransfer[0], *_dotPTransfer[1]);

    _tmp2->bindAny();
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RED, GL_FLOAT, data3);
    _tmp1->bindAny();
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RED, GL_FLOAT, data4);

    double rmsq = 0.0;
    for (int i = 0; i < _width*_height; i++) {
        float r1 = data1[i] - data3[i];
        float r2 = data2[i] - data4[i];

        rmsq += r1*r1 + r2*r2;
    }

    printf("Add sub RSQ: %f\n", rmsq);
    fflush(stdout);
}

void Fluid::testClear() {
    float *data = new float[_width*_height];

    for (int i = 0; i < _width*_height; i++)
        data[i] = frand()*2.0 - 1.0;
...
void Fluid::testScaledAdd() {
    ...
    for (int i = 0; i < _width*_height; i++) {
        data1[i] = data2[i] + alphas[0]/alphas[4]*data1[i];
    }

    scaledAdd(*_u, *_v, *_tmp1, *_dotPTransfer[0], *_dotPTransfer[1]);
    ...
    printf("Scaled add RSQ: %f\n", rmsq);
    fflush(stdout);
}

void Fluid::testPressureMatrix() {
    const float timestep = 1e-4;
    float scale = timestep/_density*1.0/_hX;
    ...
    for (int y = 0, idx = 0; y < _height; y++) {
        for (int x = 0; x < _width; x++, idx++) {
            ...
            if (x > 0) D += 1.0;
            if (y > 0) D += 1.0;
            if (x < _width - 2)  { D += 1.0; PX = -1.0; }
            if (y < _height - 2) { D += 1.0; PY = -1.0; }
            ...
            float r1 = D  - data1[idx];
            float r2 = PX - data2[idx];
            float r3 = PY - data3[idx];

            rmsq += r1*r1 + r2*r2 + r3*r3;
        }
    }

    printf("Build P Matrix RSQ: %f\n", rmsq);
}

void Fluid::testHistoPyramid() {
    int *data = new int[_width*_height];

    int total = 0;
    for (int i = 0; i < _width*_height; i++) {
        data[i] = abs(irand()) & 0xFF;
        total += data[i];
    }

    _histoCount[0]->copy(data);

    histoPyramid();

    _histoCount[_histoLevels - 1]->bindAny();
    int res;
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RED_INTEGER, GL_INT, &res);

    printf("Histo pyramid diff: %d\n", total - res);
}
...
void Fluid::update(float timestep) {
    ...
    int pAdd = addInflow(0.78, 0.05, 0.2, 0.01, 5000*_width/1920, Vec4(0.0, _tAmb, 0.0, 0.0), Vec4(1.0, 200.0, 0.0, 0.0));
